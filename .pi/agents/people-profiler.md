...
tools: read, write, edit, bash, grep, find, qmd_query, qmd_get, qmd_multi_get, qmd_status
max_turns: 30
skills:
  - obsidian-markdown
  - qmd
...
> **⚠️ Slack access unavailable under this agent.** This harness has no Slack MCP server — `slack_read_user_profile` and `slack_search_users` are absent, and every call fails. Do not attempt Slack lookups. Work only from what the caller provides: if no title/team is known, leave those fields out of the note rather than inventing them, and say which fields were unavailable in the final summary. (Slack-backed profiling remains available in Claude Code, where the Slack MCP server is registered.)
